...
from fastest to slowest is so great, we're really talking about orders of
magnitude.
...
instructions/sec, but Typed Racket does not make the top three. The next best
thing -- bare Racket vectors -- are twice as fast but still only clock in at
half the speed of flvectors, which comes out on top with a whopping 2.2M
instructions/sec. That's 4x more throughput than fully-typed matrix/array and
over 1600x untyped matrix/array. Yikes.
...
Using the specialized function directly bypasses the dynamic dispatch
mechanism and doubles throughput once more.


***** The Takeaway

If you're looking for blazing fast CPU-based matrix math, take a look at GLM
for Racket. It (can soon say it) delivers the performance of flvectors with
the convenience of popular Racket idioms.
